...
import {
  ...
  type CreatePayableErrors,
} from '@/modules/payables/presentation/schemas/payable-schema'
import { CreatePayableDto } from '@/modules/payables/domain/dto/create-payable-dto'
import {
  dateBrToIso,
  formatDateBr,
  formatDecimalInput,
...
const errors = ref<CreatePayableErrors>({})
...
function submit(): void {
  const result = validateCreatePayable(form)
  errors.value = result.errors
  if (!result.ok) return

  emit(
    'confirm',
    new CreatePayableDto({
...
<template>
  <Modal
    ...
    description="Lance um título avulso (não vinculado a uma compra)."
    size="lg"
  ...
  >
    <form class="space-y-4" @submit.prevent="submit">
      ...
      <Input
        ...
        maxlength="60"
        placeholder="Ex.: Despesas fixas (opcional)"
      >
