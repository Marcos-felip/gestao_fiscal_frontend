...
const controller = makePurchasesListController()
...
const target = ref<Purchase | null>(null)
...
function goNew(): void {
  controller.router.push({ name: routeNames.PURCHASE_NEW })
}

function goDetail(purchase: Purchase): void {
...
function canDeletePurchase(purchase: Purchase): boolean {
  return (
    canDelete.value &&
...
function askDelete(purchase: Purchase): void {
...
<template>
  ...
  <!-- Empty -->
  <div
    v-else-if="controller.purchases.value.length === 0"
  ...
  >
    ...
    <p class="mx-auto mt-1 max-w-sm text-sm text-muted-foreground">
      Registre pedidos de compra e confirme para dar entrada no estoque.
    </p>
  ...
  >
    <div class="overflow-x-auto">
      <table class="w-full min-w-[760px] text-left text-sm">
        ...
        <tbody class="divide-y divide-line-2">
          <tr
            v-for="purchase in controller.purchases.value"
            :key="purchase.id"
            class="cursor-pointer transition-colors hover:bg-muted/40"
            @click="canView && goDetail(purchase)"
          >
            ...
            <td class="px-4 py-3 text-right tabular-nums font-medium text-foreground">
              R$ {{ formatMoney(purchase.totalAmount) }}
            </td>
            <td class="px-4 py-3">
    ...
    >
      <p class="text-sm text-muted-foreground">
        ...
        {{ controller.total.value === 1 ? 'compra' : 'compras' }}
      </p>
  ...
  <ConfirmDialog
    ...
    :description="`Excluir a compra #${target?.purchaseNumber}? Esta ação não pode ser desfeita.`"
