...
import {
  validatePayReceivable,
  type PayReceivableErrors,
} from '@/modules/receivables/presentation/schemas/receivable-schema'
import { PayReceivableDto } from '@/modules/receivables/domain/dto/pay-receivable-dto'
...
const emit = defineEmits<{
  ...
  confirm: [dto: PayReceivableDto]
}>()

const amount = ref('')
const method = ref('')
const notes = ref('')
const errors = ref<PayReceivableErrors>({})
...
function submit(): void {
  const result = validatePayReceivable(
...
<template>
  ...
  >
    <form class="space-y-4" @submit.prevent="submit">
      ...
      >
        <template #label>Valor da baixa</template>
      ...
      <button
        ...
        @click="fillTotal"
      >
        Quitar total (R$ {{ formatMoney(props.balance) }})
      </button>
      ...
      <Input v-model="notes" maxlength="500" placeholder="Observações da baixa">
