...
- **CRUD em lista:** o módulo `establishments` é a referência de CRUD completo — lista em **cards** (grade), criar/editar em **página separada** (`/new`, `/:id/edit`) reusando `FormSection`, e exclusão com `ConfirmDialog` (regra: MATRIZ não pode ser excluída → botão desabilitado com `Tooltip`).
